# add PrettyTables, SpecialFunctions, Format, Statistics, DataFrames, Plots, Printf, SymPy, PyCall
...
app.layout = html_div([
    ...
    html_div([
        dcc_markdown("""\$f(x) =\$""", 
            ...
            # html_div(dcc_input(id = "function", value = "", type = "text"),
    ...
    dcc_tabs(id = "tabs", value = "limits", children =[#Can change opening tab with value
        ...
        dcc_tab(label="Plot Function", value = "functionplot", children=[
            html_div([
                ...
                html_div(dcc_input(id = "functionplot-left-endpoint", value = -2, type = "number"),
                    style=Dict("width" => "12%", "display" => "inline-block")
                ),
                html_div("Right plot endpoint: ",
                        style=Dict("width" => "9%", "display" => "inline-block")
                ),
                html_div(dcc_input(id = "functionplot-right-endpoint", value = 2, type = "number"),
                    style=Dict("width" => "12%", "display" => "inline-block")
                ),
            ]),
            html_div([
...
callback!(
    ...
    if isnothing(clicks) || ismissing(fcn) || ismissing(lim_value)
        throw(PreventUpdate())
    else # add popup if function is missing
        fcn = replace(fcn, "^" => "**")
        if lim_value == "oo"
            val = ∞
        elseif lim_value == "-oo"
        ...
        # val = lim_value == "oo" ? ∞ : parse(Float64, lim_value)
        if limit_type == "limittable"
            ...
            if lim_value == "oo" || lim_value == "-oo"
                # return limittable(sympy.parse_expr(fcn), val; rows = parse(Int64, num_rows), format = format)
                return limittable(sympy.parse_expr(fcn), val; rows = num_rows, format = format)
            else
                # return limittable(sympy.parse_expr(fcn), val; rows = parse(Int64, num_rows), dir = lim_dir, format = format)
                return limittable(sympy.parse_expr(fcn), val; rows = num_rows, dir = lim_dir, format = format)
            end
        else
            limit_result = lim(sympy.parse_expr(fcn), limit_var, val; dir = lim_dir)
            # dcc_markdown("""\\lim_{x \\to $(string(val))} f(x) = $(string(limit_result))\)""", mathjax  = true)
            dcc_markdown("""\$\\displaystyle{\\lim_{x \\to """*(string(val))*"""}} f(x) = """*(string(limit_result))*"""\$""", mathjax  = true)
            # html_div(L"\lim_{x \to "*string(val)*L"} f(x) = "*string(limit_result))
        end
    end
end
...
callback!(
    ...
    limit_type == "limittable" ? true : false
end
...
callback!(
    ...
    if isnothing(clicks) || isempty(fcn) 
        throw(PreventUpdate())
    else
        fcn = replace(fcn, "^" => "**")
        # fcn = x -> 
        ...
        # dotverticaljog = parse(Float64, dotverticaljog)
        ...
        if howmany == "single"
            # println("Let's make the signchart")
            ...
            # println("Let's make the signcharts for $fcn")
        ...
        savefig(p, figname)
        # println("Ran signchart:\n $p")
        return html_img(src = "data:image/$imageFormat;base64,$(open(base64encode, figname))")
    end
end
#getsigns
# Function plot ###################################################################################################################################
...
callback!(
    ...
    if isnothing(clicks)
        ...
        fcn = replace(fcn, "^" => "**")
        return "$(diff(sympy.parse_expr(fcn), x, n))"
    end
end
...
callback!(
    ...
    if isnothing(clicks)
        ...
        fcn = replace(fcn, "^" => "**")
        return "$(int(sympy.parse_expr(fcn), sympy.parse_expr(integral_var)))"
    end
end
...
callback!(
    ...
    if isnothing(clicks)
        ...
        fcn = replace(fcn, "^" => "**")
        if reduce == "reduced"
            return "$(N(int(sympy.parse_expr(fcn), (sympy.parse_expr(integral_var), a, b))))"
        else
            return "$(int(sympy.parse_expr(fcn), (sympy.parse_expr(integral_var), a, b)))"
        end
    end
end
